...
class vit_twoHeads_v2(nn.Module):
    def __init__(self, backbone, in_dim, num_class, num_superclass, num_fine, use_bn=False, norm_last_layer=True, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, granularity=3, feature_size=768):
        super().__init__()
        ...
        self.projector_fine = DINOHead(in_dim=feature_size//2, out_dim=num_fine, nlayers=nlayers)
    ...
    def forward(self, x):
        ...
        feat_1 =  feat[:,  0:self.feature_size//2]
        feat_2 =  feat[:,self.feature_size// 2 : self.feature_size]
        
        order_input  = torch.cat([feat_1, feat_2.detach()],1)
        species_input = feat_2

        order_proj, order_out = self.projector_super(order_input)
        species_proj, species_out = self.projector_fine(species_input)
        
        return (order_proj, order_out), (None, None ), (species_proj, species_out)

class vit_threeHeads_v2(nn.Module):
    def __init__(self, backbone, in_dim, num_class, num_superclass, num_fine, use_bn=False, norm_last_layer=True, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256, granularity=3, feature_size=768):
        super().__init__()
        self.backbone = backbone
...
class DINOHead_threeOutput(nn.Module):
    def __init__(self, in_dim, out_dim, use_bn=False, norm_last_layer=True, 
                 nlayers=3, hidden_dim=2048, bottleneck_dim=256):
        super().__init__()
        nlayers = max(nlayers, 1)
...
def info_nce_logits_smooth(features, confusion_factor, args,similarity='cos'):
    ...
    if similarity == 'cos':
        similarity_matrix = torch.matmul(features, features.T)  # shape (2B, 2B+C)
    else:
...
def info_nce_logits(features, n_views=2, temperature=1.0, device='cuda', hyp_c = 0, normalize = True, similarity='cos'):
