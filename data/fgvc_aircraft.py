...
class FGVCAircraft(Dataset):
    ...
    def __init__(self, root, class_type='variant', split='train', transform=None,
                 target_transform=None, loader=default_loader, download=False, label_level = 'species'):
        ...
        elif label_level == 'family':
            species_list = [i[1] for i in samples]
            _, family_list = get_aircraft_order_family_target(np.array(species_list))
            self.samples = [(samples[n][0], family_list[n]) for n in range(len(samples))]
        elif label_level == 'order':
            species_list = [i[1] for i in samples]
            order_list, _ = get_aircraft_order_family_target(np.array(species_list))
            self.samples = [(samples[n][0], order_list[n]) for n in range(len(samples))]
...
def get_aircraft_datasets_family(train_transform, test_transform, train_classes=range(35), prop_train_labels=0.5,
                    split_train_val=False, seed=0):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = FGVCAircraft(root=aircraft_root, transform=train_transform, split='trainval', label_level='family')
...
def get_aircraft_datasets_order(train_transform, test_transform, train_classes=range(35), prop_train_labels=0.5,
                    split_train_val=False, seed=0):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = FGVCAircraft(root=aircraft_root, transform=train_transform, split='trainval', label_level='order')
...
if __name__ == '__main__':
    ...
    x = get_aircraft_datasets_family(None, None, split_train_val=False)
