...
class CarsDataset(Dataset):
    """
        Cars Dataset
    """
    def __init__(self, train=True, limit=0, data_dir=car_root, transform=None, level = 'species'):
        ...
        if level == 'species':
            ...
            self.target = get_cars_order_family_target(self.target)
...
def get_scars_order_datasets(train_transform, test_transform, train_classes=range(160), prop_train_labels=0.8,
                    split_train_val=False, seed=0):

    np.random.seed(seed)

    # Init entire training set
    whole_training_set = CarsDataset(data_dir=car_root, transform=train_transform, train=True, level = 'order')
...
if __name__ == '__main__':
    ...
    x = get_scars_order_datasets(None, None, train_classes=range(98), prop_train_labels=0.5, split_train_val=False)
